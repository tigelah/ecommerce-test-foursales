  ...
  public PageResult<ProductDTO> search(ProductSearchQuery q) {
    try {
      ...
      if (q.name() != null && !q.name().isBlank()) {
        must.add(Query.of(b -> b.multiMatch(m -> m
          .query(q.name())
      ...
      if (q.category() != null && !q.category().isBlank()) {
        filter.add(Query.of(b -> b.term(t -> t
          .field("category")
          .value(v -> v.stringValue(q.category()))
        )));
      }

      filter.add(Query.of(b -> b.range(r -> r
        .field("stock")
        .gt(JsonData.of(0))
      )));
      ...
      SearchRequest request = SearchRequest.of(sr -> sr
        ...
        .from(q.page() * q.size())
        .size(q.size())
        .sort(s -> s.field(f -> f.field("updatedAt").order(SortOrder.Desc)))
        .query(boolQuery)
      );

      @SuppressWarnings("unchecked")
      SearchResponse<Map<String, Object>> resp =
        es.search(request, (Class<Map<String, Object>>) (Class<?>) Map.class);

      long total = resp.hits().total() != null ? resp.hits().total().value() : 0;

      List<ProductDTO> items = resp.hits().hits().stream()
        .map(hit -> {
          Map<String, Object> doc = hit.source();
          if (doc == null) return null;

          // Conversões inline, sem helpers
          UUID id = null;
          Object idRaw = doc.get("id");
          if (idRaw != null) {
            try { id = (idRaw instanceof UUID u) ? u : UUID.fromString(String.valueOf(idRaw)); }
