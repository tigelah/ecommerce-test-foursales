...
public class OrderRepositoryJpaAdapter implements OrderRepository {
  ...
  private OrderEntity toEntity(Order o) {
    return OrderEntity.builder().id(o.getId()).userId(o.getUserId()).status(o.getStatus().name()).total(o.getTotal()).createdAt(o.getCreatedAt()).paidAt(o.getPaidAt()).items(o.getItems().stream().map(it -> OrderItemEntity.builder().productId(it.productId()).quantity(it.quantity()).price(it.unitPrice()).build()).collect(Collectors.toList())).build();
  }

  private Order toDomain(OrderEntity e) {
    var items = e.getItems().stream().map(i -> new OrderItem(i.getProductId(), i.getQuantity(), i.getPrice())).collect(Collectors.toList());
    return Order.builder().id(e.getId()).userId(e.getUserId()).items(items).status(OrderStatus.valueOf(e.getStatus())).total(e.getTotal()).createdAt(e.getCreatedAt()).paidAt(e.getPaidAt()).build();
  }
}
