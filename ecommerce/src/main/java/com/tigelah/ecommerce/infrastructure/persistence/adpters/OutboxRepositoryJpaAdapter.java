...
public class OutboxRepositoryJpaAdapter implements OutboxRepository {
  ...
  public List<OutboxRecord> findNextPending(int limit) {
    return jpa.findTop50ByStatusOrderByCreatedAtAsc(Status.PENDING.name())
      .stream().map(this::toRecord).limit(limit).toList();
  }

  @Override public void markPublished(UUID id) {
    jpa.findById(id).ifPresent(e -> { e.setStatus(Status.PUBLISHED.name()); jpa.save(e); });
  }

  @Override public void markFailed(UUID id) {
    jpa.findById(id).ifPresent(e -> { e.setStatus(Status.FAILED.name()); e.setLastAttempt(Instant.now()); jpa.save(e); });
  }
