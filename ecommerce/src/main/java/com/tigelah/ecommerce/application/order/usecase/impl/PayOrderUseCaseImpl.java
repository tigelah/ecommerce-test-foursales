...
public class PayOrderUseCaseImpl implements PayOrderUseCase {
  ...
  public Order handle(PayOrderCommand cmd) {
    var order = orderRepository.findById(cmd.orderId()).orElseThrow(() -> new IllegalArgumentException("Pedido não encontrado"));

    if (!order.getUserId().equals(cmd.userId())) throw new IllegalStateException("Pedido não pertence ao usuário");

    if (order.getStatus() == OrderStatus.CANCELADO)
      throw new IllegalStateException("Pedido cancelado não pode ser pago");

    if (order.getStatus() == OrderStatus.PAGO) return order;

    order.setStatus(OrderStatus.PAGO);
    order.setPaidAt(Instant.now());
    orderRepository.update(order);

    var evt = new OrderPaidEvent(order.getId(), order.getUserId(), order.getItems().stream().map(i -> new OrderPaidEvent.Item(i.productId(), i.quantity())).collect(java.util.stream.Collectors.toList()), order.getPaidAt(), OrderPaidEvent.type(), 1);
